...
export default function HrSalaryManagement() {
  ...
  const fetchSalary = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://localhost:8080/salary/hr/month?${params.toString()}`,
        { withCredentials: true }
      );
      setResult(res.data || []);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/salary/edit/${editForm.salaryId}`,
        {
  ...
  const fetchAllSalary = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8080/salary/hr/month?month=${form.month}&year=${form.year}`,
        { withCredentials: true }
      );
      setResult(res.data || []);
      setFilteredResults(res.data || []);
